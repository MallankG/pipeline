...
def _require_dataset(db: Client, dataset_id: str):
    dataset = db.table("datasets").select("*").eq("id", dataset_id).maybe_single().execute()
    if not dataset.data:
        raise HTTPException(status_code=404, detail="Dataset not found")
    return dataset.data


def _require_version(db: Client, dataset_id: str, version_id: str):
    version = (
...
def get_dataset(dataset_id: str, db: Client = Depends(get_db)):
    return _require_dataset(db, dataset_id)
